...
int record_command_buffers(GlobalData *p_win, uint32_t,
                           uint32_t frame_inx) {
    ...
    result = vkBeginCommandBuffer(p_win->cmd_buffer[frame_inx],
                                  &cmd_begin_info);
    res--;
    ...
    vkCmdBeginRenderPass(p_win->cmd_buffer[frame_inx],
                         &rndr_begin_info,
                         VK_SUBPASS_CONTENTS_INLINE);
    vkCmdBindPipeline(p_win->cmd_buffer[frame_inx],
    ...
    vkMapMemory(p_win->device, p_win->vert_memory,
                1024 * frame_inx + 1024, sizeof(float), 0, &del);
    *del = p_win->ex_data.deltas / 256.f;
    vkUnmapMemory(p_win->device, p_win->vert_memory);

    vkCmdBindDescriptorSets(
      p_win->cmd_buffer[frame_inx], VK_PIPELINE_BIND_POINT_GRAPHICS,
      p_win->graphics_pipeline_layout, 0, 1,
      p_win->descriptor_sets + frame_inx, 0, NULL);
    ...
    vkCmdBindVertexBuffers(p_win->cmd_buffer[frame_inx], 0, 1,
                           &p_win->vert_buff, &offsets);
...
int draw_frame(GlobalData *p_win) {
    ...
    result = vkWaitForFences(p_win->device, 1,
                             p_win->in_flight_fences +
                               p_win->curr_frame_in_flight,
                             VK_TRUE, UINT64_MAX);

    res--;
    if (result == VK_ERROR_OUT_OF_DATE_KHR ||
        !p_win->img_swap_extent.width ||
        !p_win->img_swap_extent.height) {
        if (recreate_swapchain(p_win) < 0)
            return res;
        return 0;
    }

    vkResetFences(p_win->device, 1,
                  p_win->in_flight_fences +
                    p_win->curr_frame_in_flight);

    uint32_t img_inx;
    vkAcquireNextImageKHR(
      p_win->device, p_win->curr_swapchain.swapchain, UINT64_MAX,
      p_win->image_available_semaphores[p_win->curr_frame_in_flight],
      VK_NULL_HANDLE, &img_inx);

    vkResetCommandBuffer(
      p_win->cmd_buffer[p_win->curr_frame_in_flight], 0);
    record_command_buffers(p_win, img_inx,
                           p_win->curr_frame_in_flight);
    ...
    VkSubmitInfo submit_info = {
        ...
        .pCommandBuffers =
          &(p_win->cmd_buffer[p_win->curr_frame_in_flight]),
        .signalSemaphoreCount = COUNT_OF(signal_semaphores),
        .pSignalSemaphores = signal_semaphores
    };

    result = vkQueueSubmit(
      p_win->graphics_queue, 1, &submit_info,
      p_win->in_flight_fences[p_win->curr_frame_in_flight]);
    res--;
    if (result != VK_SUCCESS)
        return res;

    VkSwapchainKHR swapchains[] = { p_win->curr_swapchain.swapchain };
    ...
    p_win->curr_frame_in_flight =
      (p_win->curr_frame_in_flight + 1) % p_win->max_frames_in_flight;
...
LRESULT CALLBACK wnd_proc(HWND h_wnd, UINT msg, WPARAM wparam,
    ...
    if (msg == WM_CREATE) {
        ...
        res--;
        if (vkCreateWin32SurfaceKHR(g_vk_instance,
                                    &surface_create_info,
        ...
        p_win->uniform_buffer =
          malloc(p_win->max_frames_in_flight * sizeof(VkBuffer));
        res--;
        ...
        vkGetPhysicalDeviceMemoryProperties(p_win->phy_device,
                                            &mem_props);
        ...
        vkGetBufferMemoryRequirements(p_win->device, p_win->vert_buff,
                                      &buff_mem_req1);
        ...
        vkGetBufferMemoryRequirements(
          p_win->device, p_win->uniform_buffer[0], &buff_mem_req2);
        ...
        for (int i = 0; i < mem_props.memoryTypeCount; ++i) {
            if (((1 << i) & buff_mem_req1.memoryTypeBits) &&
                ...
                (reqs ==
                 (reqs & mem_props.memoryTypes[i].propertyFlags))) {
                mem_inx = i;
                break;
            }
        }
        ...
        VkMemoryAllocateInfo alloc_info = {
            ...
            .allocationSize =
              1024 + 1024 * p_win->max_frames_in_flight,
        ...
        for (int i = 0; i < p_win->max_frames_in_flight; ++i) {
            vkBindBufferMemory(p_win->device,
                               p_win->uniform_buffer[i],
                               p_win->vert_memory, 1024 * (i + 1));
        }

        vkBindBufferMemory(p_win->device, p_win->vert_buff,
                           p_win->vert_memory, 0);

        float *ptr;
        vkMapMemory(p_win->device, p_win->vert_memory, 0, 1024, 0,
                    &ptr);
        ...
        result = vkCreateDescriptorPool(
          p_win->device, &descriptor_pool_info,
          p_win->p_host_alloc_calls, &p_win->descriptor_pool);
        ...
        p_win->descriptor_sets = malloc(p_win->max_frames_in_flight *
        ...
        for (int i = 0; i < p_win->max_frames_in_flight; ++i) {
            ...
            result = vkAllocateDescriptorSets(
              p_win->device, &descriptor_sets_alloc,
        ...
        VkWriteDescriptorSet *descriptor_write_structs =
          malloc(p_win->max_frames_in_flight *
        ...
        VkDescriptorBufferInfo *descriptor_buff_infos =
          (VkDescriptorBufferInfo *)(descriptor_write_structs +
                                     p_win->max_frames_in_flight);

        for (int i = 0; i < p_win->max_frames_in_flight; ++i) {
            descriptor_buff_infos[i] =
        ...
        vkUpdateDescriptorSets(p_win->device,
                               p_win->max_frames_in_flight,
                               descriptor_write_structs, 0, NULL);
    ...
    if (p_win) {
        ...
        if (msg == WM_DESTROY) {
            ...
            vkDestroyDescriptorPool(p_win->device,
            ...
            p_win->old_swapchain.img_count =
              p_win->curr_swapchain.img_count;
        ...
        if (msg == WM_PAINT) {
            draw_frame(p_win);
            p_win->ex_data.deltas =
              (p_win->ex_data.deltas + 257) % (2 * 256) - 256;
        }
        if (msg == WM_TIMER) {
